/**
 * An Event is a gathering that happens on a particular date. It has a defined
 * name, starting time, and ending time.
 * 
...
public class Event {
	...
	public Event(String eventName, int year, int month, int day, int start,
			int end) {
		this.name = eventName;
		this.year = year;
		this.month = month;
	 ...
	 * 
	 * @return 1 if this event is later than otherEvent, -1 if this event is
	 *         earlier than otherEvent, or 0 if the two events occur on exactly
	 *         the same day with the exact same START time.
	 */
