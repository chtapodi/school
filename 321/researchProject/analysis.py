...
def chunkByDist(dataSet, minDist) :
	...
	while len(dataStack)>0 :
		...
		for entry in dataStack :
			...
			if dist<=minDist :
				chunk.append(entry)
				printVar="X {0}".format(i)
			else :
				rejects.append(entry)

		##printING CODE
		#fprint("looking for",d0)
		# 	print("{3}{0}: {1}:  {2}".format(d1, len(dataStack),  dist, printVar))
		# 	i=i+1
		#
		# dataStack=rejects
		# print("taken:{0}".format(len(chunk)))
		# for n in chunk :
		# 	print("  {0}".format(n[2]))
		#
		# print("remains:{0}".format(len(dataStack)))
		# for n in dataStack :
		# 	print("  {0}".format(n[2]))
